...
describe('Modal', () => {
  ...
  it('hides close button when showCloseButton is false', () => {
    ...
    expect(
      screen.queryByRole('button', { name: /关闭/i })
  ...
  it('calls onClose when overlay is clicked', async () => {
    ...
    // The overlay is the element with role="dialog"
    const overlay = screen.getByRole('dialog');

    // Use fireEvent for more direct control over the click event
  ...
  it('applies correct size class', () => {
    ...
    let modal = dialog.querySelector('[tabindex="-1"]'); // The modal content div has tabindex="-1"
    expect(modal?.className).toMatch(/small/);

    rerender(<Modal {...defaultProps} size="large" />);
    modal = screen.getByRole('dialog').querySelector('[tabindex="-1"]');
    expect(modal?.className).toMatch(/large/);
  });
  ...
  it('applies custom className', () => {
    ...
    expect(modal?.className).toMatch(/custom-modal/);
  });
