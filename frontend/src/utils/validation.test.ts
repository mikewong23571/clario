...
describe('ProjectValidationRules', () => {
  describe('projectName', () => {
    it('should validate project name correctly', () => {
      ...
      expect(validateField(longName, rules).error).toBe('项目名称不能超过 50 个字符');
      
      // Test invalid characters
      expect(validateField('test@project', rules).isValid).toBe(false);
      expect(validateField('test@project', rules).error).toBe('项目名称只能包含字母、数字、中文、下划线、连字符和空格');
  ...
  describe('projectDescription', () => {
    it('should validate project description correctly', () => {
      ...
      expect(validateField(longDesc, rules).error).toBe('项目描述不能超过 200 个字符');
...
describe('validateField', () => {
  it('should validate field with multiple rules', () => {
    ...
    expect(validateField('abcdefghijk', rules).error).toBe('不能超过 10 个字符');
...
describe('validateForm', () => {
  it('should validate entire form', () => {
    const schema = {
      ...
      email: [ValidationRules.required(), ValidationRules.pattern(/^\S+@\S+$/, '无效的邮箱格式')],
    };
